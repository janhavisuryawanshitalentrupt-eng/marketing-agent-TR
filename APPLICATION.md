...
| **Magazine** | Generate a branded, festive **multi-page magazine PDF** ("Talentrupt Times") from the team's REAL photos + provided stats. An issue form (title/edition/theme/editorial) + a COVER champion (employee + headline + tagline + stat callouts) + SPOTLIGHTS (employee + office + blurb + stats) → `POST /api/magazine/generate` renders each page as a full PIL image and assembles them into one PDF (`generation/magazine.py`). Cover = framed real-photo portrait + stat pills; editorial = LLM-written note; spotlights = 2/page (circular photo + chips + blurb); + closing. Saved as a `magazine` Asset, owner-scoped. | all |
