...
| **Folders** | **Reference photo library** of employees (name + role + **one or MORE photos** each). Add an employee with several photos at once (multi-select), or hit **"Add photos"** on any existing card to attach more later; a lightbox shows all their shots (switch/download/delete extras; the cover is deletable only by removing the employee). Multiple photos are stored in an **`employee_photos`** table (the cover stays on `Employee.photo_path`) and **rotated at random** when the person is featured, so repeated posts vary. Endpoints: `POST /api/folders/{id}/employees` (multi-file, first = cover), `POST /api/employees/{id}/photos`, `DELETE /api/employees/{id}/photos/{photo_id}`; each employee serializes a `photos[]` + `photo_count`. Feature them in **Chat** by typing **`@`their name** → a post with their **real** photo (never an AI face). | all |
